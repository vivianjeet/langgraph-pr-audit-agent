...
def test_pr_approved_true_when_count_positive():
    with patch.object(gg.subprocess, "run", return_value=_proc(0, "2\n")):
        assert gg.pr_is_human_approved() is True

def test_pr_approved_false_when_zero():
    with patch.object(gg.subprocess, "run", return_value=_proc(0, "0\n")):
